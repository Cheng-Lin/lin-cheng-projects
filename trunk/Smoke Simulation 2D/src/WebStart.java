...
//			System.out.println(fs.d[(n + 2) / 2][y] + " ");
//		}
//		System.out.println("--");
    ...
    public void updateLocation(MouseEvent e)
    {
        ...
        if (button == 1) fs.dOld[i][j] = 100;
        if (button == 3 && e.getID() == MouseEvent.MOUSE_DRAGGED)
        {
            fs.uOld[i][j] = (x - xOld) * 5;
            fs.vOld[i][j] = (y - yOld) * 5;
        }
    }
    ...
    public void printStuff(float input[][])
    {
    	...
    	for (int x = 1; x <= n; x++)
    	{
    		for (int y = 1; y <= n; y++)
    		{
    			scaled = input[x][y] * 10000f;
    			if (scaled > 999) scaled = 999;
    			if (scaled < -999) scaled = -999;
    			
    			if (scaled >-1.1f && scaled < 1.1f)
    				System.out.print(".... ");
    			else if (scaled < 0)
